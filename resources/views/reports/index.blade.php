    ...
    <div x-show="activeTab === 'formA'">
        ...
        <div class="bg-blue-50 border border-blue-200 rounded p-4 mb-4">
            <div class="flex">
                ...
                <div class="ml-3">
                    <h3 class="text-sm font-medium text-blue-800">Institutional Configuration Required</h3>
                    <div class="mt-2 text-sm text-blue-700">
                        <p>Please complete the institutional information below. This data will be included in all CHED report exports.</p>
                    </div>
                </div>
            </div>
        </div>
        ...
        <form method="POST" action="{{ route('reports.save-institutional') }}" class="bg-gray-50 p-6 rounded border">
            @csrf
            <h3 class="text-lg font-semibold mb-4 text-gray-900">FORM A: Institutional Information</h3>
            
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                <!-- HEI Information -->
                <div class="space-y-4">
                    <h4 class="font-medium text-gray-700">HEI Information</h4>
                    
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Name of HEI *</label>
                        <input type="text" name="hei_name" value="{{ old('hei_name', $institutionalData['hei_name'] ?? '') }}" 
                               class="w-full border rounded p-2" required>
                    </div>

                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">HEI Campus (if applicable)</label>
                        <input type="text" name="hei_campus" value="{{ old('hei_campus', $institutionalData['hei_campus'] ?? '') }}" 
                               class="w-full border rounded p-2">
                    </div>

                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Address of HEI *</label>
                        <textarea name="hei_address" rows="2" class="w-full border rounded p-2" required>{{ old('hei_address', $institutionalData['hei_address'] ?? '') }}</textarea>
                    </div>

                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Name of HEI President *</label>
                ...
                <div class="space-y-4">
                    <h4 class="font-medium text-gray-700">Sports Director Information</h4>
                    
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Name of Sports Director *</label>
                        <input type="text" name="sports_director_name" value="{{ old('sports_director_name', $institutionalData['sports_director_name'] ?? '') }}" 
                               class="w-full border rounded p-2" required>
                    </div>

                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Email of Sports Director *</label>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                <div class="space-y-4">
                    <h4 class="font-medium text-gray-700">RA 11180 Contact Person</h4>
                    ...
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Email of Contact Person *</label>
            ...
            <!-- Intramurals Section -->
            <div class="mb-6">
                <h4 class="font-medium text-gray-700 mb-3">Intramurals Activities</h4>
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <label class="flex items-center">
                        <input type="checkbox" name="departmental_intramurals" value="1" 
                               {{ old('departmental_intramurals', $institutionalData['departmental_intramurals'] ?? false) ? 'checked' : '' }}
                               class="mr-2">
                        Departmental Intramurals
                    </label>
                    <label class="flex items-center">
                        <input type="checkbox" name="interdepartmental_intramurals" value="1"
                               {{ old('interdepartmental_intramurals', $institutionalData['interdepartmental_intramurals'] ?? false) ? 'checked' : '' }}
                               class="mr-2">
                        Interdepartmental Intramurals
                    </label>
                    <label class="flex items-center">
                        <input type="checkbox" name="intercampus_intramurals" value="1"
                               {{ old('intercampus_intramurals', $institutionalData['intercampus_intramurals'] ?? false) ? 'checked' : '' }}
                               class="mr-2">
                        Inter-campus Intramurals
                    </label>
                </div>
            </div>
            ...
            <!-- Facilities Section -->
            <div class="mb-6">
                <h4 class="font-medium text-gray-700 mb-3">Sports Facilities</h4>
                <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                    @foreach([
                        'gymnasium' => 'Gymnasium',
                        'multipurpose_hall' => 'Multi-purpose Hall',
                        'quadrangle' => 'Quadrangle',
                        'athletes_dormitory' => 'Athletes Dormitory',
                        ...
                        'sports_medical_facility' => 'Sports Medical Facility'
                    ] as $key => $label)
                    <label class="flex items-center">
                        <input type="checkbox" name="facilities[{{ $key }}]" value="1"
                               {{ old('facilities.' . $key, $institutionalData['facilities'][$key] ?? false) ? 'checked' : '' }}
                               class="mr-2">
                        {{ $label }}
                    </label>
                    @endforeach
            ...
            <!-- Other Facilities -->
            <div class="mb-6">
                <label class="block text-sm font-medium text-gray-700 mb-1">Other Training Venues and Athletic Facilities</label>
                <textarea name="other_facilities" rows="2" class="w-full border rounded p-2" 
            ...
            <table class="min-w-[2600px] w-full border-collapse text-sm text-gray-800 text-center">
                ...
                <tbody>
                    @foreach($sports as $index => $sport)
                        @php
                            $sp = $sportsPrograms[$sport->sport_id] ?? [];
                        @endphp
                        <tr class="odd:bg-gray-50 even:bg-white hover:bg-yellow-50">
                            <td class="border p-1">{{ $index + 1 }}</td>

                            <!-- ITEM CODE (could be sport_code if available) -->
                            <td class="border p-1">{{ $sport->sport_code ?? ('B1-SPORT-' . ($index+1)) }}</td>

                            <!-- B1-SPORTS-1: Sport selection (keeps selection available) -->
                            <td class="border p-1">
                                <select name="sports[{{ $sport->sport_id }}][sport_id]" class="w-full border rounded p-1 text-sm">
                                    <option value="">Select Sport</option>
                                    @foreach($sports as $sOpt)
                                        <option value="{{ $sOpt->sport_id }}" {{ (int)old('sports.' . $sport->sport_id . '.sport_id', $sport->sport_id) === (int)$sOpt->sport_id ? 'selected' : '' }}>
                                            {{ $sOpt->sport_name }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <td class="border p-1">
                                <select name="sports[{{ $sport->sport_id }}][category]" class="w-full border rounded p-1 text-sm">
                                    <option value="">Select</option>
                                    <option value="Men" {{ old('sports.' . $sport->sport_id . '.category', $sp['category'] ?? '') == 'Men' ? 'selected' : '' }}>Men</option>
                                    <option value="Women" {{ old('sports.' . $sport->sport_id . '.category', $sp['category'] ?? '') == 'Women' ? 'selected' : '' }}>Women</option>
                                    <option value="Mixed" {{ old('sports.' . $sport->sport_id . '.category', $sp['category'] ?? '') == 'Mixed' ? 'selected' : '' }}>Mixed</option>
                                </select>
                            </td>

                            <!-- B2-ASSOC-1..B2-ASSOC-3 (first) -->
                            <td class="border p-1">
                                <select name="sports[{{ $sport->sport_id }}][assoc_1]" class="w-full border rounded p-1 text-sm">
                                    <option value="">Select</option>
                                    <option value="Yes" {{ old('sports.' . $sport->sport_id . '.assoc_1', $sp['assoc_1'] ?? '') == 'Yes' ? 'selected' : '' }}>Yes</option>
                                    <option value="No" {{ old('sports.' . $sport->sport_id . '.assoc_1', $sp['assoc_1'] ?? '') == 'No' ? 'selected' : '' }}>No</option>
                                </select>
                            </td>

                            <td class="border p-1">
                                <select name="sports[{{ $sport->sport_id }}][assoc_2]" class="w-full border rounded p-1 text-sm">
                                    <option value="">Select</option>
                                    <option value="Yes" {{ old('sports.' . $sport->sport_id . '.assoc_2', $sp['assoc_2'] ?? '') == 'Yes' ? 'selected' : '' }}>Yes</option>
                                    <option value="No" {{ old('sports.' . $sport->sport_id . '.assoc_2', $sp['assoc_2'] ?? '') == 'No' ? 'selected' : '' }}>No</option>
                                </select>
                            </td>

                            <td class="border p-1">
                                <select name="sports[{{ $sport->sport_id }}][assoc_3a]" class="w-full border rounded p-1 text-sm">
                                    <option value="">Select</option>
                                    <option value="Yes" {{ old('sports.' . $sport->sport_id . '.assoc_3a', $sp['assoc_3a'] ?? '') == 'Yes' ? 'selected' : '' }}>Yes</option>
                                    <option value="No" {{ old('sports.' . $sport->sport_id . '.assoc_3a', $sp['assoc_3a'] ?? '') == 'No' ? 'selected' : '' }}>No</option>
                                </select>
                            </td>

                            <!-- B2-ASSOC-3 (duplicate) named assoc_3b to keep unique input names -->
                            <td class="border p-1">
                                <select name="sports[{{ $sport->sport_id }}][assoc_3b]" class="w-full border rounded p-1 text-sm">
                                    <option value="">Select</option>
                                    <option value="Yes" {{ old('sports.' . $sport->sport_id . '.assoc_3b', $sp['assoc_3b'] ?? '') == 'Yes' ? 'selected' : '' }}>Yes</option>
                                    <option value="No" {{ old('sports.' . $sport->sport_id . '.assoc_3b', $sp['assoc_3b'] ?? '') == 'No' ? 'selected' : '' }}>No</option>
                                </select>
                            </td>

                            <!-- B2-ASSOC-4: TEXT AREA (comma separated associations) -->
                            <td class="border p-1">
                                <textarea name="sports[{{ $sport->sport_id }}][assoc_other]" rows="2" class="w-full border rounded text-sm p-1" placeholder="e.g., Association 1, Association 2">{{ old('sports.' . $sport->sport_id . '.assoc_other', $sp['assoc_other'] ?? '') }}</textarea>
                            </td>

                            <!-- B3-LEAGUE-1..10 -->
                            <!-- We'll render them as alternating Active (Yes/No) and Number fields as per input guide -->
                            @for($i = 1; $i <= 10; $i++)
                                @php
                                    $isActive = $i % 2 == 1; // odd indexes: Active (Yes/No); even: Number
                                ...
                                @if($isActive)
                                    <td class="border p-1">
                                        <select name="sports[{{ $sport->sport_id }}][{{ $activeName }}]" class="w-full border rounded p-1 text-sm">
                                            <option value="">Select</option>
                                            <option value="Yes" {{ old('sports.' . $sport->sport_id . '.' . $activeName, $sp[$activeName] ?? '') == 'Yes' ? 'selected' : '' }}>Yes</option>
                                            <option value="No" {{ old('sports.' . $sport->sport_id . '.' . $activeName, $sp[$activeName] ?? '') == 'No' ? 'selected' : '' }}>No</option>
                                        </select>
                                    </td>
                                @else
                                    <td class="border p-1">
                                        <input type="number" min="0" name="sports[{{ $sport->sport_id }}][{{ $countName }}]"
                                               value="{{ old('sports.' . $sport->sport_id . '.' . $countName, $sp[$countName] ?? '') }}"
                            ...
                            @for($i = 1; $i <= 3; $i++)
                                <td class="border p-1">
                                    <select name="sports[{{ $sport->sport_id }}][well_{{ $i }}]" class="w-full border rounded p-1 text-sm">
                                        <option value="">Select</option>
                                        <option value="Yes" {{ old('sports.' . $sport->sport_id . '.well_' . $i, $sp['well_' . $i] ?? '') == 'Yes' ? 'selected' : '' }}>Yes</option>
                                        <option value="No" {{ old('sports.' . $sport->sport_id . '.well_' . $i, $sp['well_' . $i] ?? '') == 'No' ? 'selected' : '' }}>No</option>
                                    </select>
                                </td>
                            @endfor
                            ...
                            @for($i = 1; $i <= 7; $i++)
                                <td class="border p-1">
                                    <select name="sports[{{ $sport->sport_id }}][cpd_{{ $i }}]" class="w-full border rounded p-1 text-sm">
                                        <option value="">Select</option>
                                        <option value="Yes" {{ old('sports.' . $sport->sport_id . '.cpd_' . $i, $sp['cpd_' . $i] ?? '') == 'Yes' ? 'selected' : '' }}>Yes</option>
                                        <option value="No" {{ old('sports.' . $sport->sport_id . '.cpd_' . $i, $sp['cpd_' . $i] ?? '') == 'No' ? 'selected' : '' }}>No</option>
                                    </select>
                                </td>
                            @endfor
...
<div x-show="activeTab === 'formC'">
    ...
    <!-- FORM D: Sports Personnel Table -->
    <div x-show="activeTab === 'formD'">
        <div class="bg-blue-50 p-3 border rounded-t">
            <h3 class="font-semibold text-blue-900">FORM D: Sports Personnel</h3>
            <p class="text-sm text-blue-700">Total Records: {{ $coaches->count() }}</p>
        </div>
        <table class="w-full border-collapse border border-gray-300 bg-white">
            <thead class="bg-gray-200">
                <tr>
                    <th class="border border-gray-300 px-3 py-2 text-left text-xs font-medium text-gray-700 uppercase tracking-wider">Name</th>
                    <th class="border border-gray-300 px-3 py-2 text-left text-xs font-medium text-gray-700 uppercase tracking-wider">Age</th>
                    <th class="border border-gray-300 px-3 py-2 text-left text-xs font-medium text-gray-700 uppercase tracking-wider">Sex</th>
                    <th class="border border-gray-300 px-3 py-2 text-left text-xs font-medium text-gray-700 uppercase tracking-wider">Sports Program</th>
                    <th class="border border-gray-300 px-3 py-2 text-left text-xs font-medium text-gray-700 uppercase tracking-wider">Position Title</th>
                    <th class="border border-gray-300 px-3 py-2 text-left text-xs font-medium text-gray-700 uppercase tracking-wider">Employment Status</th>
                    <th class="border border-gray-300 px-3 py-2 text-left text-xs font-medium text-gray-700 uppercase tracking-wider">Monthly Salary</th>
                    <th class="border border-gray-300 px-3 py-2 text-left text-xs font-medium text-gray-700 uppercase tracking-wider">Years Experience</th>
                    <th class="border border-gray-300 px-3 py-2 text-left text-xs font-medium text-gray-700 uppercase tracking-wider">Certifications</th>
                </tr>
            </thead>
            <tbody class="bg-white divide-y divide-gray-200">
                @forelse($coaches as $coach)
                <tr class="hover:bg-gray-50">
                    <td class="border border-gray-300 px-3 py-2 text-sm">{{ $coach->full_name }}</td>
                    <td class="border border-gray-300 px-3 py-2 text-sm text-center">{{ $coach->age ?? '-' }}</td>
                    <td class="border border-gray-300 px-3 py-2 text-sm text-center">{{ $coach->gender ?? '-' }}</td>
                    <td class="border border-gray-300 px-3 py-2 text-sm">{{ $coach->sport->sport_name ?? '-' }}</td>
                    <td class="border border-gray-300 px-3 py-2 text-sm">{{ $coach->position_title ?? 'Coach' }}</td>
                    ...
                    <td class="border border-gray-300 px-3 py-2 text-sm text-center">{{ $coach->years_experience ?? '-' }}</td>
                    <td class="border border-gray-300 px-3 py-2 text-sm">{{ $coach->certifications ?? '-' }}</td>
                </tr>
                @empty
                <tr>
                    <td colspan="9" class="border border-gray-300 px-3 py-4 text-center text-sm text-gray-500">
...
<div x-show="activeTab === 'formG'">
    ...
    <!-- Individual Export Buttons for Each Form -->
    <div class="mt-6 grid grid-cols-1 md:grid-cols-4 gap-4">
        <!-- Form A Export -->
        <div class="flex flex-col gap-2">
            <h4 class="font-semibold text-gray-700 text-sm">FORM A Export</h4>
            <div class="flex gap-2">
                <a href="{{ route('reports.export.form', ['form' => 'A', 'format' => 'pdf']) }}?{{ http_build_query(request()->all()) }}" 
                   class="flex-1 bg-red-600 hover:bg-red-700 text-white px-3 py-2 rounded text-sm flex items-center justify-center gap-1">
                    <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/>
                    </svg>
                    PDF
                </a>
                <a href="{{ route('reports.export.form', ['form' => 'A', 'format' => 'csv']) }}?{{ http_build_query(request()->all()) }}" 
                   class="flex-1 bg-green-600 hover:bg-green-700 text-white px-3 py-2 rounded text-sm flex items-center justify-center gap-1">
                    <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/>
                    </svg>
                    CSV
        ...
        <!-- Form B Export -->
        <div class="flex flex-col gap-2">
            <h4 class="font-semibold text-gray-700 text-sm">FORM B Export</h4>
            <div class="flex gap-2">
                <a href="{{ route('reports.export.form', ['form' => 'B', 'format' => 'pdf']) }}?{{ http_build_query(request()->all()) }}" 
                   class="flex-1 bg-red-600 hover:bg-red-700 text-white px-3 py-2 rounded text-sm flex items-center justify-center gap-1">
                    <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/>
                    </svg>
                ...
                <a href="{{ route('reports.export.form', ['form' => 'B', 'format' => 'csv']) }}?{{ http_build_query(request()->all()) }}" 
                   class="flex-1 bg-green-600 hover:bg-green-700 text-white px-3 py-2 rounded text-sm flex items-center justify-center gap-1">
                    <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/>
                    </svg>
        ...
        <!-- Form C Export -->
        <div class="flex flex-col gap-2">
            <h4 class="font-semibold text-gray-700 text-sm">FORM C Export</h4>
            <div class="flex gap-2">
                <a href="{{ route('reports.export.form', ['form' => 'C', 'format' => 'pdf']) }}?{{ http_build_query(request()->all()) }}" 
                   class="flex-1 bg-red-600 hover:bg-red-700 text-white px-3 py-2 rounded text-sm flex items-center justify-center gap-1">
                    <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/>
                    </svg>
                ...
                <a href="{{ route('reports.export.form', ['form' => 'C', 'format' => 'csv']) }}?{{ http_build_query(request()->all()) }}" 
                   class="flex-1 bg-green-600 hover:bg-green-700 text-white px-3 py-2 rounded text-sm flex items-center justify-center gap-1">
                    <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/>
                    </svg>
        ...
        <!-- Form D Export -->
        <div class="flex flex-col gap-2">
            <h4 class="font-semibold text-gray-700 text-sm">FORM D Export</h4>
            <div class="flex gap-2">
                <a href="{{ route('reports.export.form', ['form' => 'D', 'format' => 'pdf']) }}?{{ http_build_query(request()->all()) }}" 
                   class="flex-1 bg-red-600 hover:bg-red-700 text-white px-3 py-2 rounded text-sm flex items-center justify-center gap-1">
                    <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/>
                    </svg>
                ...
                <a href="{{ route('reports.export.form', ['form' => 'D', 'format' => 'csv']) }}?{{ http_build_query(request()->all()) }}" 
                   class="flex-1 bg-green-600 hover:bg-green-700 text-white px-3 py-2 rounded text-sm flex items-center justify-center gap-1">
                    <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/>
                    </svg>
        ...
        <!-- Form E Export -->
        <div class="flex flex-col gap-2">
            <h4 class="font-semibold text-gray-700 text-sm">FORM E Export</h4>
            <div class="flex gap-2">
                <a href="{{ route('reports.export.form', ['form' => 'E', 'format' => 'pdf']) }}?{{ http_build_query(request()->all()) }}" 
                class="flex-1 bg-red-600 hover:bg-red-700 text-white px-3 py-2 rounded text-sm flex items-center justify-center gap-1">
                    <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/>
                    </svg>
                ...
                <a href="{{ route('reports.export.form', ['form' => 'E', 'format' => 'csv']) }}?{{ http_build_query(request()->all()) }}" 
                class="flex-1 bg-green-600 hover:bg-green-700 text-white px-3 py-2 rounded text-sm flex items-center justify-center gap-1">
                    <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/>
                    </svg>
         ...
         <!-- Form F Export -->
        <div class="flex flex-col gap-2">
            <h4 class="font-semibold text-gray-700 text-sm">FORM F Export</h4>
            <div class="flex gap-2">
                <a href="{{ route('reports.export', ['format' => 'pdf']) }}?{{ http_build_query(request()->all()) }}&form=F" 
                class="flex-1 bg-red-600 hover:bg-red-700 text-white px-3 py-2 rounded text-sm flex items-center justify-center gap-1">
                    <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/>
                    </svg>
                ...
                <a href="{{ route('reports.export', ['format' => 'csv']) }}?{{ http_build_query(request()->all()) }}&form=F" 
                class="flex-1 bg-green-600 hover:bg-green-700 text-white px-3 py-2 rounded text-sm flex items-center justify-center gap-1">
                    <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/>
                    </svg>
         ...
         <!-- Form G Export -->
    <div class="flex flex-col gap-2">
        <h4 class="font-semibold text-gray-700 text-sm">FORM G Export</h4>
        <div class="flex gap-2">
            ...
            <a href="{{ route('reports.export.form', ['form' => 'G', 'format' => 'csv']) }}" 
               class="flex-1 bg-green-600 hover:bg-green-700 text-white px-3 py-2 rounded text-sm flex items-center justify-center gap-1">
                <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/>
                </svg>
...
function calculateBudgetTotals() {
    ...
    fundInputs.forEach((input, index) => {
    ...
    // Update display - FORCE update to test
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Initial calculation after delay to ensure DOM is ready
    setTimeout(function() {
        calculateBudgetTotals();
    }, 500);
    
    // Set up observer to recalculate when FORM E becomes visible
...
document.addEventListener('DOMContentLoaded', () => {
    document.querySelector('#benefitsTable').addEventListener('change', e => {
